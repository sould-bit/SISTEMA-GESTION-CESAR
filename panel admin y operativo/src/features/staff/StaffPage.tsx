...
export const StaffPage = () => {
    ...
    const handleDeleteUser = async (user: User) => {
        ...
        try {
            await StaffService.deleteUser(user.id);
            loadData();
        } catch (error) {
            console.error(error);
    ...
    const handleRestoreUser = async (user: User) => {
        if (!confirm(`¿Estás seguro de reactivar al usuario ${user.username}?`)) return;

        try {
            await StaffService.updateUser(user.id, { is_active: true });
            loadData();
        } catch (error) {
            console.error(error);
            ...
            <div className="flex justify-between items-center">
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors text-sm border ${showInactive
                            ? 'bg-blue-500/10 text-blue-400 border-blue-500/20'
                            : 'bg-gray-800 text-gray-400 border-gray-700 hover:text-white'}`}
                        title={showInactive ? "Ocultar usuarios inactivos" : "Ver historial de usuarios inactivos"}
                    >
                        <span className="material-symbols-outlined text-[18px]">
                            {showInactive ? 'visibility' : 'visibility_off'}
                        </span>
                        {showInactive ? 'Inactivos Visibles' : 'Ver Inactivos'}
                    </button>
                    {viewMode === 'users' ? (
                        <button
                            onClick={() => {
                    ...
                    <div className="bg-card-dark border border-border-dark rounded-xl overflow-hidden">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-border-dark">
                                {users.filter(user => showInactive || user.is_active).map((user) => (
                                    <tr key={user.id} className="hover:bg-white/5 transition-colors group">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex gap-2">
                                                {/* Ocultar acciones si es admin */}
                                                {user.role !== 'admin' && user.role !== 'owner' && (user.role_name && user.role_name.toLowerCase() !== 'administrador') ? (
                                                    <>
                                                        <button
                                                            onClick={() => handleEditUser(user)}
                                                            className="p-1 rounded hover:bg-yellow-500/20 text-yellow-500 transition-colors"
