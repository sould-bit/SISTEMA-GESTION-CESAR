...
> *"Consecutivos M-XXX, L-XXX, D-XXX en backend"* (✅ Verificado en Tests)
...
- ✅ Prefijos M-/L-/D- **IMPLEMENTADOS** (Verificado: `M-` para dine_in por defecto)
- ✅ Máquina de estados para flujo de pedidos
- ✅ Soporte para delivery (customer_id, delivery_address, etc.)

> El sistema genera correctamente prefijos como `M-001` para pedidos en mesa.
...
### Alta Prioridad 🔴3
1. **Crear endpoints de domiciliarios:**
2. **Crear endpoints de domiciliarios:**
   - `/orders/{id}/assign-delivery`
   - `/delivery/my-orders`
   - `/delivery/orders/{id}/delivered`
2. **Configurar Celery** para que el servicio de impresión funcione
